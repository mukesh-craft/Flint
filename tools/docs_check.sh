...
check "t_flow.fl" 43 "$(lines tests/t_flow.fl)"
check "t_funcs.fl" 15 "$(lines tests/t_funcs.fl)"
check "t_types.fl" 23 "$(lines tests/t_types.fl)"
check "smoke total" 103 "$(( $(lines tests/t_hello.fl) + $(lines tests/t_arith.fl) + $(lines tests/t_flow.fl) + $(lines tests/t_funcs.fl) + $(lines tests/t_types.fl) ))"
check "run.sh" 30 "$(lines tests/run.sh)"
check "run_tutorial.sh" 18 "$(lines tests/run_tutorial.sh)"
check "test_registry.sh" 43 "$(lines tests/test_registry.sh)"
check "runners total" 91 "$(( $(lines tests/run.sh) + $(lines tests/run_tutorial.sh) + $(lines tests/test_registry.sh) ))"
